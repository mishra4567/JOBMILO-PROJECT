...
$id = $_POST['id'];
$select = "UPDATE job_details SET status = 'approved' WHERE id = '$id' ";
$resut = mysqli_query($con, $select);
header("location:job_post_request.php");
}
...
$id = $_POST['id'];
$select = "DELETE  FROM job_details   WHERE id = '$id' ";
$resut = mysqli_query($con, $select);
header("location:job_post_request.php");
}
...
<h5 class="text-center  text-white bg-warning  col-md-8 d-flex align-item-center justify-content-center mb-4">Pending List </h5>

<?php
include("db/db.php");

$sel = "SELECT p.id, p.company_name, p.phone, p.photo, c.id, c.j_title, c.salary, c.description, c.work_exp, c.english, c.education, c.role, c.emp_type, c.shift FROM employer_detail p INNER JOIN job_details c ON p.id=c.user_id Where status='pending' ";
...
<input onclick="return confirm
" type="submit" name="delete" value="Decline" onclick ="return confirm('Are you sure');" class="btn btn-danger">
...
<h5 class="text-center  text-white bg-success mb-4 ">Approved List </h5>
<?php
include("db/db.php");

$sel = "SELECT p.id, p.company_name, p.phone, p.photo, c.id, c.j_title, c.salary, c.description, c.work_exp, c.english, c.education, c.role, c.emp_type, c.shift FROM employer_detail p INNER JOIN job_details c ON p.id=c.user_id Where status='approved' ";
